...
function Navbar(props, { defaultActive }) {
  ...
  useEffect(() => {
    const activePage = NavbarItems.findIndex(page => 
      getRoutePath(page.route) === getRoutePath(location.pathname));
  ...
  return(
    <>
      {
        NavbarItems.map((page, index) => printSelections(page, index))
      }
    </>
  );
}
